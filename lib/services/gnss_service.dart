...
class GnssService extends ChangeNotifier {
  ...
  final SimulatedLocationService _simulatedLocation =
  ...
  Map<String, PositionData> get peerPositions =>
      Map.unmodifiable(_peerPositions);
  ...
  Future<bool> startPositioning() async {
    ...
    try {
      if (_useSimulation) {
        ...
        // Start position updates with high accuracy (Google Maps style)
        // bestForNavigation provides the most accurate GPS data including speed
        const LocationSettings locationSettings = LocationSettings(
          accuracy: LocationAccuracy.bestForNavigation,
          distanceFilter: 0, // Update on any movement for accurate speed
          timeLimit: Duration(seconds: 5), // Faster timeout for navigation
        );
    ...
    Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      _mockMeasurements = List.generate(8, (index) {
        return MockGnssMeasurementModel(
          svid: index + 1,
          constellationType: index < 4
              ? 1
              : (index < 6 ? 6 : 1), // GPS and NavIC mix
          timeNanos: DateTime.now().millisecondsSinceEpoch * 1000000,
          carrierFrequencyHz: index < 4
              ? 1575.42e6
              : 1176.45e6, // L1 and L5 frequencies
  ...
  GnssPositioningMode _determinePositioningMode() {
    ...
    if (_peerPositions.isNotEmpty) {
      return GnssPositioningMode
          .dgps; // Cooperative positioning similar to DGPS
    }
  ...
  PositionData? _calculateIPRCorrection(PositionData basePosition) {
    ...
    for (final peer in nearbyPeers) {
      ...
      final weight =
          1.0 / (1.0 + distance / 100.0); // Closer peers have higher weight
