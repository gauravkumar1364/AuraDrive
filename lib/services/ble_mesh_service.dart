...
class BLEMeshService {
  final String?
  appServiceUuid; // optional UUID to match devices running the same app

  BLEMeshService({this.appServiceUuid});
  // Device discovery
  final Map<String, BLEDevice> _discoveredDevices = {};
  final Map<String, BluetoothCharacteristic> _characteristics = {};
  final Set<String> _autoConnectAttempted = {};
  ...
  static const int rssiThreshold =
      -70; // dBm - devices closer than this can cluster
  ...
  static final Guid meshServiceUuid = Guid(
    '0000180a-0000-1000-8000-00805f9b34fb',
  );
  static final Guid meshCharacteristicUuid = Guid(
  ...
  Future<void> startScanning() async {
    ...
    try {
      ...
      // Listen to scan results before starting to avoid missed events
      FlutterBluePlus.scanResults.listen((results) {
        for (ScanResult result in results) {
          // Log advertisement for debugging
          try {
            final adv = result.advertisementData;
            // If appServiceUuid is provided and the advertisement contains it, auto-connect once
            if (appServiceUuid != null) {
              final advUuids = adv.serviceUuids
                  .map((u) => u.toString().toLowerCase())
                  .toList();
              if (advUuids.contains(appServiceUuid!.toLowerCase())) {
                final deviceId = result.device.remoteId.toString();
                if (!_autoConnectAttempted.contains(deviceId)) {
                  _autoConnectAttempted.add(deviceId);
      ...
      // Start scanning with slightly longer windows to improve discovery on busy environments
      await FlutterBluePlus.startScan(
        timeout: const Duration(seconds: 8),
        androidUsesFineLocation: true,
      );

      // Auto restart scanning periodically while _isScanning is true
  ...
  void _createOrJoinCluster(List<BLEDevice> nearbyDevices) {
    ...
    if (nearbyDevices.length >= minClusterSize - 1) {
      ...
      print(
        'Created cluster ${_myCluster!.id} with ${clusterMembers.length} members',
      );
    }
  }
  ...
  Future<void> connectToDevice(String deviceId) async {
    ...
    try {
      await bleDevice.device.connect(timeout: const Duration(seconds: 5));
  ...
  List<BLEDevice> getDiscoveredDevices() {
    return _discoveredDevices.values.where((d) => d.isActive).toList()
