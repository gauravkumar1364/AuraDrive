...
class CollisionDetectionService extends ChangeNotifier {
  ...
  // Thresholds
  static const double crashThreshold = 39.2; // 4G in m/s²
  static const double hardBrakingThreshold = -6.0; // m/s²
  static const double sharpTurnThreshold = 4.0; // m/s²
  static const double proximityWarningDistance = 50.0; // meters
  static const double collisionWarningDistance = 25.0; // meters
  ...
  Stream<AccelerationData> get accelerationStream => _accelerationController.stream;
  ...
  /// Check for crash detection based on acceleration
  void _checkForCrash(AccelerationData acceleration) {
    if (acceleration.indicatesCrash) {
      final alert = CollisionAlert(
        ...
        message: 'CRASH DETECTED! High impact acceleration: ${acceleration.magnitude.toStringAsFixed(1)} m/s²',
      );
      
      _addAlert(alert);
      debugPrint('CollisionDetectionService: Crash detected! Magnitude: ${acceleration.magnitude}');
    }
  }
  
  /// Check for sudden movements (hard braking, sharp turns)
  void _checkForSuddenMovements(AccelerationData acceleration) {
    if (acceleration.indicatesHardBraking) {
      final alert = CollisionAlert(
        alertId: _generateAlertId(),
        sourceDeviceId: 'current_device',
        ...
        message: 'Hard braking detected: ${acceleration.y.toStringAsFixed(1)} m/s²',
      );
    ...
    if (acceleration.indicatesSharpTurn) {
      final alert = CollisionAlert(
        ...
        timestamp: DateTime.now(),
        expiresAt: DateTime.now().add(const Duration(seconds: 30)),
        message: 'Sharp turn detected: ${acceleration.x.toStringAsFixed(1)} m/s²',
      );
  ...
  CollisionAlert? _assessCollisionRisk(VehicleData vehicle) {
    ...
    final timeToCollision = vehicle.timeToCollision(_createMockCurrentVehicle(currentPosition));
    ...
    if (distance <= criticalDistance) {
      ...
      message = 'CRITICAL: Vehicle extremely close (${distance.toStringAsFixed(1)}m)';
    } else if (distance <= collisionWarningDistance) {
      riskLevel = CollisionRiskLevel.high;
      alertType = AlertType.collision;
      message = 'COLLISION WARNING: Vehicle approaching (${distance.toStringAsFixed(1)}m)';
    } else if (distance <= proximityWarningDistance) {
      riskLevel = CollisionRiskLevel.medium;
      alertType = AlertType.proximity;
      message = 'Proximity warning: Vehicle nearby (${distance.toStringAsFixed(1)}m)';
    } else {
      return null; // No risk
    }
  ...
  VehicleData _createMockCurrentVehicle(PositionData position) {
    return VehicleData(
      ...
      acceleration: _currentAcceleration ?? AccelerationData(
        x: 0.0,
        y: 0.0,
        z: 9.8,
        magnitude: 9.8,
  ...
  double _calculateRelativeSpeed(VehicleData otherVehicle) {
    ...
    return otherVehicle.velocity.speed; // Assume head-on approach for worst case
  }
  ...
  void addVehicleToTracking(VehicleData vehicle) {
    ...
    _trackedVehicles.removeWhere((key, value) => value.timestamp.isBefore(cutoffTime));
  ...
  void _addAlert(CollisionAlert alert) {
    ...
    final existingSimilar = _activeAlerts.any((existing) =>
  ...
  void _cleanupExpiredAlerts(Timer timer) {
    ...
    if (_activeAlerts.length != initialCount) {
      notifyListeners();
      debugPrint('CollisionDetectionService: Cleaned up ${initialCount - _activeAlerts.length} expired alerts');
    }
  }
  ...
  bool get isAggressiveDriving {
    ...
    final recentAccelerations = _accelerationHistory
        .where((data) => DateTime.now().difference(data.timestamp).inSeconds < 30)
        .toList();
